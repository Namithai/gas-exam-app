 ...
 * 変更8: 出題画面の「選択肢を選んでください」をポップアップから画面内メッセージに変更
 *        （スマホではポップアップに長いURLが出て内容が読みにくいため）
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* 未回答などの注意メッセージ（ポップアップだとスマホで長いURLが出るため画面内に表示する） */
        .quiz-caution { display: none; margin: 10px 0 0; padding: 12px 14px; border-radius: 8px; background: #fdecea; border: 2px solid #dc3545; color: #c0392b; font-weight: bold; font-size: 14px; text-align: center; }
        .quiz-caution--show { display: block; }
    ...
    function loadDashboard() {
      loadDashboardTerm();
        ...
        function renderQuiz() {
            ...
            html += '<p id="quizCaution" class="quiz-caution"></p>';
        ...
        function showQuizCaution(message) {
            var el = document.getElementById('quizCaution');
            if (!el) return;
            el.textContent = message;
            el.classList.add('quiz-caution--show');
            el.scrollIntoView({ block: 'center' });
        }
        function clearQuizCaution() {
            var el = document.getElementById('quizCaution');
            if (el) el.classList.remove('quiz-caution--show');
        }

        function onNextClicked() {
            if (answers[currentQuizIndex].length === 0) {
                showQuizCaution('選択肢を選んでください');
                return;
            }
            clearQuizCaution();
